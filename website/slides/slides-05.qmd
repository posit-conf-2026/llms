...
:::: {.columns style="--code-font-size: 0.72em; margin-top: 1.2em;"}

::: {.column width="50%"}
### R: vitals

```{.r code-line-numbers="|2|3|4-6|1-7|9"}
task <- Task$new(
  dataset = cases,
  solver = plot_solver,
...
```{.python code-line-numbers="|2|3|4|1-5|7"}
task = Task(
    dataset=cases,
...
The solver receives each input and produces the model output.
...
| **Human review** | A person reads and grades the output. | High control for the user, but slow, expensive, and largely impractical. |
:::
...
::: {.fragment}
![](assets/bluffbench-example.png){fig-alt="A scatter plot of miles per gallon against horsepower in a secretly altered mtcars dataset. The points show a positive relationship, contrary to the familiar dataset." width="67%"}
:::
...
![](assets/bluffbench-results.png){fig-alt="A horizontal stacked bar chart comparing models on bluffbench. Blue shows correct plot interpretations and orange shows incorrect interpretations, with separate panels for thinking and non-thinking models." width="82%"}

...
::: notes
OpenAI follows a similar pattern. they used to do mini, nano, pro, but now have planet / space themed names.
:::
...
[![](assets/artificial-analysis-intelligence-vs-cost-2026-09-11.png){width="111%" style="max-width: none; transform: translateX(-5%);" fig-alt="Scatter plot of Artificial Analysis Intelligence Index against cost per task for 16 models available through Posit AI. The upper-left green quadrant marks models with higher intelligence and lower cost."}][artificial-analysis-intelligence-cost]

::: notes
Up and to the left is better. That means higher intelligence at a lower cost per task.
...
chat <- chat_posit("claude-haiku-4-5")
chat$chat(
...
chat = chatlas.ChatPosit("claude-haiku-4-5")
chat.chat(
    content_pdf_file("financial-report.pdf"),
    "What's my tax liability for 2024?"
)
...
::: notes
Or use content_pdf_url() to pass a PDF from a URL. Same pattern, different source.
:::
...
chat <- chat_posit(model = "anthropic/claude-haiku-4-5")
chat$chat(
...
chat = chatlas.ChatPosit(model="anthropic/claude-haiku-4-5")
chat.chat(
    content_pdf_url("http://pdf.secdatabase.com/1757/0001104659-25-042659.pdf"),
    "Describe Tesla’s executive compensation and stock award programs."
)
...
They do not work in an interactive interpreter, where `Field(description=...)`
is more reliable.
:::
...
::: notes
Quick recap of structured output from the previous section: one prompt in, one structured result out.
Emphasize that so far we've only ever processed one string at a time.
:::

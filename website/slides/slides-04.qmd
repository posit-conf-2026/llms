...
::: notes
Posit AI is the provider we will use today. It hosts models from several
...
1. Speed: Current measurements range from about 50 to 300 output tokens/sec: GLM-5.3-Flash 51, Claude Sonnet 5 75, GPT-5.6 Terra 117, and Gemini 3.8 Flash 292.
1. Cost: Current list prices range from $0.15 input / $0.50 output per 1M tokens for GLM-5.3-Flash to $5 / $25 for Claude Opus 5. Output tokens usually cost more than input tokens.
:::
...
Open weight model doesn't necessarily mean local.
:::
...
Posit AI, not directly to Moonshot AI.
:::
...
them a provider name to use that provider's default model. Posit currently
selects Claude Sonnet 5 by default.
:::
...
::: notes
Use `"provider/model"` to select both at once. This example still uses Posit AI
as the provider and selects Kimi K3 as the model.
:::
